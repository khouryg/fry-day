...
Apple Health integration is optional. Profile read access is requested when you import age and skin type; dietary Vitamin D write access is requested separately when you enable session export. Imported profile values remain in the same protected, backup-excluded local file. If you enable export, newly saved session estimates are written to Health’s dietary Vitamin D category. Earlier sessions can be exported individually from history. Entries include the session dates and metadata identifying the values as modeled sun-exposure estimates, not food or supplement intake. Health and other authorized apps may aggregate these entries with dietary intake. Fry Day does not read dietary vitamin D history. Disabling export or deleting a session locally does not delete previously exported Health entries; manage those in Health. Health data follows your device’s Health sharing and synchronization settings. You can manage Health permissions in Health or Settings. Revoking access does not itself delete profile values already imported; clear the age in How It Works and change skin type using the main screen or delete the app.
...
With notification permission, Fry Day schedules a local, best-effort exposure warning for an active session using its available UV forecast and inherited exposure model. The warning is updated when new information becomes available and canceled when the session ends. Forecasts and iOS notification delivery can be delayed; warnings are not a guarantee against sunburn. Fry Day does not operate a push notification server.
